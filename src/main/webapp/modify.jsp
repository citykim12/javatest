...
<html>
	<%
		MDao mDao = new MDao();
		String id = (String)session.getAttribute("sessoin_id");
		MDto mDto = mDao.memberOneSelect(id);
		//mDao.selectMemberOne(id);
	%>
	<head>
		<meta charset="UTF-8">
		<title>회원정보수정</title>
		...
		<script type="text/javascript">
			...
			function formCheck(){
				if($("#pw").val()!=<%=mDto.getPw() %>){
					alert("패스워드가 일치하지 않으면 수정할 수 없습니다~! 패스워드를 다시 입력해주세요~!");
					$("#pw").focus();
					return false;
				}
	...
	<body>
	  ...
	  <form action="./modifyOk.jsp" id="form" name="form">
	    <label>아이디</label>
	    <input type="text" id="id" name="id" value="<%=mDto.getId() %>" ><br>
	    <label>비밀번호</label>
